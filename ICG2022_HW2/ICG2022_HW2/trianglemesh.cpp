	...
	while (getline(infile, sline)) //從指定文件逐行讀取
	{
		...
		if (sline[0] == 'm') { //進入mtl檔案開始讀取
			...
			while (getline(inmtlfile, mtlLine)) {
				...
				for (int i = 0; i < 3; i++) {
					getline(inmtlfile, mtlLine);
					istringstream sincoff(mtlLine);//用空格做切割
					GLfloat c1, c2, c3;
					string coff;
					sincoff >> coff >> c1 >> c2 >> c3;
					mtlLine = "";
					if (i == 0) {
			...
			for (int i = 0; i < dicMat.size(); i++) {
				...
				cout << "matKs()" << dicMat[i]->GetKs().x << "," << dicMat[i]->GetKs().y << "," << dicMat[i]->GetKs().z << endl;
			}*/
			lastf = 0;
		}
...
void TriangleMesh::Draw(SubMesh obj) {
	...
	glDrawElements(GL_TRIANGLES, obj.GetnumVertexIndices(), GL_UNSIGNED_INT, 0);
